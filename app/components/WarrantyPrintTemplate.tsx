...
useEffect(() => {
  if (!isPreview) return;
  const updateScale = () => {
    const availableWidth = window.innerWidth < 640 ? window.innerWidth - 90 : window.innerWidth - 80;
...
style={isPreview && scale < 1 ? {
        ...
        <div className="border border-slate-300 rounded-lg mb-2 flex flex-col overflow-hidden">
          ...
          <div className="grid grid-cols-12 divide-x divide-y divide-slate-300 text-[8.5px] bg-white">
             ...
             <div className="col-span-3 p-1.5 flex flex-col justify-center"><span className="text-slate-500 mb-0.5 text-[7.5px]">Telefone:</span><span className="font-bold text-[#2B323D] text-[9.5px]">{Array.from(new Set([customer?.whatsapp, customer?.phone].filter(Boolean))).join(' / ') || '---'}</span></div>
